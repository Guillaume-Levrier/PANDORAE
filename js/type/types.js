...
// ========= HYPHOTYPE =========
const hyphotype = (id) => {                 // When called, draw the anthropotype
...
var corpusRequests = [];
    ...
    var tooltipTitle = "<strong>"+datajson.name.toUpperCase()+"</strong></br>"+
                         "<span>"+datajson.content.endpoint+"</span></br>";
      ...
      var startCorpus = {
              ...
              body: {"method":"start_corpus","params":[datajson.content.corpus,datajson.content.password]}, 
  ...
  corpusRequests.push(rpn(startCorpus))
    ...
    var getCorpusStats = {
            ...
            uri: datajson.content.endpoint+'/api/',                  // URI to be accessed
            headers: {'User-Agent': 'Request-Promise'},    // User agent to access is Request-promise
            body: {"method":"get_webentities_stats","params":[datajson.content.corpus]}, 
            json: true                
        };
...
console.log(status)
    ...
    console.log(error)
    field.value = "error - invalid dataset";
    ipcRenderer.send('console-logs',"Hyphotype error: dataset " +id+" is invalid.");
  });
...
grouped.forEach(d=>{d.key=d.date;d.value=d.values.length});
...
window.addEventListener("touchmove",e=>testZoom)
  ...
  function zoomed() {
    ...
    let rangeRatio = x.range()[1]/x2.range()[1];
