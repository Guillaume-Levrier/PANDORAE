        ...
        try {
            ...
            for (var i = 0; i < articles.length; ++i) {
                ...
                pushedArticle.publicationTitle =
                    articles[i]["prism:publicationName"];
                ...
                if (articles[i].hasOwnProperty("altmetricData")) {
                    enrichment.altmetric = JSON.stringify(
                        articles[i].altmetricData
                    );
                }
            ...
            (err, locatedCities) => {
                ...
                for (var i = 0; i < article.length - 1; i++) {
                    ...
                    if (
                        ...
                        for (
                            let j = 0;
                            j < article[i].affiliation.length;
                            j++
                        ) {
                            // Iterate on item's available affiliation
                            if (
                                article[i].affiliation[j].hasOwnProperty(
                                    "affiliation-country"
                                )
                            ) {
                    ...
                    for (let l = 0; l < cityIndex.length; l++) {
                        if (d.hasOwnProperty("affiliation")) {
                            for (var k = 0; k < d.affiliation.length; k++) {
                                ...
                                let country =
                                    d.affiliation[k]["affiliation-country"]; // Extract affiliation city
                ...
                ipcRenderer.send(
                    "chaeros-notification",
                    "Affiliations geolocated"
                ); //Send success message to main process
                ipcRenderer.send("pulsar", true);
                ipcRenderer.send(
                    "console-logs",
                    "scopusGeolocate successfully added geolocations on " +
                        dataset
                );
...
const biorxivRetriever = (query) => {
    ...
    if (doi.length > 0) {
        baseUrl =
            baseUrl +
            "%20doi%3A" +
            document.getElementById("biorxiv-doi").value;
    }
    if (author.length > 0) {
        baseUrl =
            baseUrl +
    ...
    requestArray.forEach((req) => {
        let requestContent = {
            type: "request",
            model: "biorxiv-content-injector",
    ...
    ipcRenderer.on("artoo", (event, message) => {
        message.content.forEach((d) =>
            doiBuffer.push(
                d.replace("doi: https://doi.org/", "").replace(" ", "")
            )
        );
        count++;
        ipcRenderer.send(
            ...
            .then((res) => {
                ...
                articles.forEach((d) => {
                    ...
                    while (pos !== -1) {
                        ...
                        let auth = {
                            creatorType: "author",
                            lastName: d.authors.substring(
                                pos,
                                d.authors.indexOf(",", pos)
                            ),
            ...
            limiter
                ...
                .then((ctResponses) => {
                    for (let i = 0; i < ctResponses.length; i++) {
                        ctResponses[
                            i
                        ].FullStudiesResponse.FullStudies.forEach((study) =>
        ...
        .then((zoteroItemsResponse) => {
            ...
            zoteroItemsResponse.forEach((f) => {
                ...
                for (var i = 0; i < f.meta.numItems; i += 100) {
                    ...
                    var zoteroVersion =
                        "/items/top?&v=3&format=csljson&start=" +
                        i +
                        "&limit=100&key=";
                ...
                limiter
                    .schedule(() =>
                        Promise.all(itemRequests.map((d) => fetch(d)))
                    )
                    .then((responses) =>
