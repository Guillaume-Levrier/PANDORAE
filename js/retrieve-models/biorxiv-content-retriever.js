...
window.addEventListener('load', e=> {
  ...
  let sendInterval = setInterval(() => {
    ...
    if (selfWinId>0 && charWinId>0){ // make sure that this chaeros process ID is known 
               
      try{
        ipcRenderer.send('biorxiv-retrieve',{type:"biorxiv-content",content:DOIs,charWindId:charWinId});
        } catch (err){
